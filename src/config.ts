...
export interface Config {
  ...
  /**
   * Your solution-sync GitHub repo, as "owner/repo" — a repo *you* chose to
   * store packaged solutions in (desc + stub + tests), written by `leet sync`
   * / `pull-solutions` and read by `mark-solved`. Overridable per-command by an
   * explicit repo argument, and via the LEET_SYNC_REPO env var. Distinct from
   * `neetcodeRepo` below — this one you own and `leet` writes to.
   */
  syncRepo?: string;
  /**
   * Your NeetCode.io GitHub sync repo, as "owner/repo" (e.g.
   * "you/neetcode-submissions-xxxx") — auto-generated by neetcode.io, read-only
   * to `leet`. Used as the default source for `import --adapter neetcode` /
   * `leet mark-solved`'s NeetCode-folder reading. Overridable per-command by an
   * explicit repo argument, and via the LEET_NEETCODE_REPO env var.
   */
  neetcodeRepo?: string;
  /**
...
export const CONFIG_FIELDS: readonly ConfigField[] = [
  ...
  {
    key: "neetcodeRepo",
    label: "NeetCode repo (owner/repo)",
    kind: "text",
    fallback: "$LEET_NEETCODE_REPO, else unset",
  },
  {
    key: "roadmapSubset",
...
/**
 * NeetCode.io GitHub sync repo ("owner/repo"): explicit CLI arg >
 * $LEET_NEETCODE_REPO > config `neetcodeRepo` > null. Distinct from
 * `resolveSyncRepo` — this is the auto-generated, read-only NeetCode submissions
 * repo, not the user's own solution-sync repo.
 */
export function resolveNeetcodeRepo(
  arg: string | undefined,
  cfg: Config,
  env: Env = process.env,
): string | null {
  return arg || env.LEET_NEETCODE_REPO || cfg.neetcodeRepo || null;
}
