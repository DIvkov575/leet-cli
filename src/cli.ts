...
import { scaffoldContent, scaffoldFilename, refreshHarness } from "./scaffold.ts";
...
import { resolveCompileFlags } from "./runner.ts";
...
Usage:
  ...
  leet fix-harness [id|slug]       Regenerate a stale/missing test harness, keeping your solution code (omit id to fix every file in --dir)
...
/**
 * `leet fix-harness [id|slug] [--dir <dir>]` — regenerate the test harness for
 * one or every scaffolded `.cpp` in <dir>, from current live LeetCode data and
 * current harness-generation code, while leaving the user's actual `class
 * Solution { ... }` body untouched. For healing files that predate a harness
 * fix: a signature that only recently gained support (missing harness
 * entirely), or a harness that's merely stale (present, but generated by
 * older code — e.g. missing the args-printing line). Skips (with a reason)
 * any file whose class body can't be confidently extracted, rather than risk
 * discarding real code — see `extractSolutionClass`.
 */
async function cmdFixHarness(p: Parsed): Promise<void> {
  const key = p.positionals[0];
  const config = await loadConfig();
  const dir = resolveSolutionsDir(p.values.dir as string | undefined, config);

  const targets: { path: string; slug: string }[] = [];
  if (key) {
    const local = await findProblemAnywhere(key);
    const slug = local?.slug ?? key;
    const glob = new Bun.Glob(`*-${slug}.cpp`);
    for await (const f of glob.scan(dir)) targets.push({ path: `${dir}/${f}`, slug });
    if (targets.length === 0) throw new UserError(`no scaffolded .cpp for "${key}" in ${dir}`);
  } else {
    const glob = new Bun.Glob(`*.cpp`);
    for await (const f of glob.scan(dir)) {
      const m = /^\d+-(.+)\.cpp$/.exec(f);
      if (m) targets.push({ path: `${dir}/${f}`, slug: m[1]! });
    }
    if (targets.length === 0) throw new UserError(`no scaffolded .cpp files found in ${dir}`);
  }
  ...
  for (const { path, slug } of targets) {
    ...
    try {
      const r = await fetchProblem(slug, { withSnippets: true, withContent: true });
      const fresh = refreshHarness(existing, {
        id: r.id,
        title: r.title,
        slug: r.slug,
        difficulty: r.difficulty,
      ...
      if (fresh === null) {
        console.log(`skip  ${path}: no recognizable class body to preserve`);
        skipped++;
      ...
      console.log(`skip  ${path}: ${err instanceof Error ? err.message : String(err)}`);
      skipped++;
    }
  }
  console.log(`\n${fixed} fixed, ${unchanged} already current, ${skipped} skipped.`);
}
...
async function main(): Promise<number> {
  ...
  switch (command) {
    ...
    case "test":
      ...
      return 0;
    case "fix-harness":
      await cmdFixHarness(parse(rest, { dir: { type: "string" } }));
      return 0;
    case "push":
      await cmdPush(
        parse(rest, {
